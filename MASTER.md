...
### Production deployment — live at https://runadvisor.muze.digital
- **Actually deployed and running**, not just scripted: DigitalOcean droplet, non-root `deploy` user (passwordless sudo) with its own read-only GitHub deploy key. `deploy/install.sh` bootstraps a fresh droplet — Docker, 2GB swapfile, ufw (SSH/80/443 only), clone, build, start. Full walkthrough in `deploy/README.md`. Live topology (droplet IP, domain, key setup) is also saved to memory for future sessions.
- Domain sits behind **Cloudflare's proxy** (discovered live, not assumed — public DNS resolves to Cloudflare edge IPs). Caddy's automatic Let's Encrypt via HTTP-01 doesn't work through that, so HTTPS uses a **Cloudflare Origin Certificate** instead (15-year validity, scoped to just the one subdomain) — `docker-compose.prod.yml` mounts it from `deploy/certs/` (gitignored). Cloudflare SSL/TLS mode must be Full (strict). Verified live by connecting directly to the droplet's IP and confirming Caddy presents that exact cert.
- Base `docker-compose.yml` binds the backend's port to `127.0.0.1` only, in both dev and prod — never reachable except through Caddy.
- **CI/CD**: `.github/workflows/deploy.yml` SSHes in and redeploys (`git pull` + `docker compose up -d --build` + `alembic upgrade head`) on every push to `master`, plus manual `workflow_dispatch`. Uses a *third* dedicated SSH keypair (distinct from the user's personal key and the droplet's read-only GitHub→droplet deploy key) — this one is GitHub Actions→droplet, private half in the `DEPLOY_SSH_KEY` repo secret, droplet IP in `DEPLOY_HOST`. Verified the key authenticates and has passwordless sudo docker access before wiring it into the workflow.
- Also live and verified end-to-end on production: Telegram Login Widget for browser access (see below) and the Garmin workout sync feature.

### Web login (Telegram Login Widget)
- The Mini App's `initData` auth only works inside Telegram. Opening the site directly in a browser needed a separate path — added the classic Telegram Login Widget (`data-auth-url` **redirect** mode, not the popup+JS-callback mode, which is fragile — real popup-blocker failure hit and fixed during testing). Maps to the *same* `telegram_id`-keyed `User` row as the Mini App, so a browser session and the Mini App session are the same identity: same Garmin link, same chat history, same everything.
- New endpoints: `GET /api/auth/config` (bot username, fetched live via `bot.get_me()`), `POST /api/auth/telegram-login` (validates the widget's HMAC scheme — `secret_key = SHA256(bot_token)`, different from initData's `HMAC-SHA256(bot_token, key="WebAppData")`). Both schemes verified against Telegram's actual docs and round-trip tested.
- Requires the bot's domain registered via `@BotFather` → `/setdomain`, or the widget shows "Bot domain invalid" (reproduced locally against `localhost` on purpose to confirm the failure mode, then resolved on the real domain).
- Found and fixed live: the callback page's effect could double-fire (parent re-render handed it a new `onSuccess` reference before the route changed, re-triggering the effect pre-unmount), causing two near-simultaneous upserts for the same `telegram_id` → `IntegrityError` → 500. Fixed on both ends: a `useRef` guard so the POST only ever fires once, *and* made `_get_or_create_user` an atomic `INSERT ... ON CONFLICT` instead of SELECT-then-INSERT (the real fix — race-safe regardless of what any caller does). Verified with a concurrent 3-way `asyncio.gather` hitting the same `telegram_id`: all three resolved cleanly to one row, no errors.
